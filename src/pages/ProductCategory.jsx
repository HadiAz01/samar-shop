...
export default function ProductCategory() {
  ...
  const { data: description, isPending: descIsPending } =
    useCategoryDescription(categoryParam);
  const { data, isPending } = useProductsCategory(categoryParam);
  ...
  useEffect(() => {
    isPending === false &&
      (setFilteredData(data), setTopic(data[0].categoryFn));
  }, [isPending, data]);
    ...
    <>
      ...
      <Breadcrumb
        links={[
          ...
          { id: 2, to: "/prod", title: topic },
        ]}
      />
      <div className="container">
        {isPending && descIsPending ? (
          <>
